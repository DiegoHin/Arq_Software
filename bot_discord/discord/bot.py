...
@bot.command(name='play', help='Juega una partida de Tic-Tac-Toe con otro miembro del canal!!\n\nEjemplo: !play <Fila> <Columna>; Fila, Columna: {1,2,3}')
async def tictac(ctx):
    message =  ctx.message.content
    channelMQ.basic_publish(exchange='cartero', routing_key="ttt", body=message)

@bot.command(name='play-again', help='Nueva partida de Tic-Tac-Toe!')
async def tictac(ctx):
    message =  ctx.message.content
    channelMQ.basic_publish(exchange='cartero', routing_key="ttt", body=message)        
